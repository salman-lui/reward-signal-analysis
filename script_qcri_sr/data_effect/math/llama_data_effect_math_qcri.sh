...
export BASE_MODEL=/export/alt-ai-agent/salman/hf_model/Llama-3.2-3B-Instruct # Path of your base policy model # CHANGE THIS

# Set save directory based on debug mode
if [ "$DEBUG" = "True" ]; then
export SAVE_DIR="/export/alt-ai-agent/salman/debug_save" # Debug save directory
else
export SAVE_DIR="/export/alt-ai-agent/salman/reward-signal-results/data_effect/math/llama3b" # Production save directory # CHANGE THIS 
fi

export EXPERIMENT_NAME=${EXPERIMENT_NAME:-llama_math_8} # Can be overridden by SLURM script

TOTAL_EPOCHS=${TOTAL_EPOCHS:-496} # Can be overridden by SLURM script (496 for 8/16/32, 248 for 64, etc.) 
SAVE_FREQ=200
TEST_FREQ=20
...
if [ "$DEBUG" = "True" ]; then
TRAIN_DATA_PATH=${TRAIN_DATA_PATH:-"$(pwd)/data/math/train/llama-3b/llama_sky_math_8_upsample.parquet"}
...
TRAIN_DATA_PATH=${TRAIN_DATA_PATH:-"$(pwd)/data/math/train/llama-3b/llama_sky_math_8_upsample.parquet"} # Can be overridden by SLURM script
