...
Usage:
    ...
    3. Set MODEL to the model name in the difficulty dictionary
    4. Run: python parquet_data_splitting_train.py

Output: Creates qwen_sky_math_{size}.parquet files with balanced difficulty distribution.
"""
...
for idx, row in df.iterrows():
    ...
    if isinstance(difficulty_dict, dict) and MODEL in difficulty_dict:
        ...
        if diff_score in DIFFICULTY_RANGE:
...
for size in SAMPLE_SIZES:
    ...
    for idx in all_indices:
        diff_score = int(df.loc[idx, 'difficulty'][MODEL])
        diff_counts[diff_score] += 1
