...
if [ "$TRAINING_MODE" = "MODE1" ] || [ "$TRAINING_MODE" = "MODE2" ]; then
    export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-"4,5,6,7"}  # 4 GPUs to match reference
fi
...
if [ "$TRAINING_MODE" = "MODE1" ]; then
    export DEBUG=True
    export EXPERIMENT_NAME=debug_majority_vote_ttrl
    export PROJECT_NAME='debug_majority_vote_ttrl'
    export RAY_HEAD_PORT=6388
elif [ "$TRAINING_MODE" = "MODE2" ]; then
    export DEBUG=False
    export EXPERIMENT_NAME=prod_majority_vote_ttrl
    export PROJECT_NAME='prod_majority_vote_ttrl'
    export RAY_HEAD_PORT=6388
else  # MODE3
    export DEBUG=False
    export EXPERIMENT_NAME=remote_majority_vote_ttrl
    export PROJECT_NAME='remote_majority_vote_ttrl'
...
if [ "$TRAINING_MODE" = "MODE1" ]; then
    ...
    export BASE_MODEL=/local2/salman/model/Qwen2.5-Math-7B-Instruct                     # Debug: Qwen2.5-Math-7B
elif [ "$TRAINING_MODE" = "MODE2" ]; then
...
if [ "$TRAINING_MODE" = "MODE1" ]; then
    ...
    BASE_SAVE_PATH="/local2/salman/co_train_qwen_2_5_math/only_majority_vote_training"
else
    BASE_SAVE_PATH="/checkpoints/salsrahm-sandbox/novel_hybrid/only_majority_vote_training"
fi
