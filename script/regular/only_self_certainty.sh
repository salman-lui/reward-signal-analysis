...
cd /home/salman/verl
export REWARD_MODEL_TYPE=INTUITOR  
...
# INTUITOR Configuration - Self-Certainty Based Training
# No external reward model needed - using intrinsic self-certainty
...
BASE_SAVE_PATH="/local2/salman/co-training/full_unsupervised/self_certainity"
PROJECT_NAME='intuitor_self_certainty'
EXPERIMENT_NAME='salsrahm_intuitor_novel'
...
if [ "$DEBUG" = "True" ]; then
    TRAIN_BATCH_SIZE=18                                     # Debug: Small batch divisible by 6 (18*4=72, 72%6=0)
    PPO_MINI_BATCH=18                                       # Debug: Match train batch size
else
    TRAIN_BATCH_SIZE=180                                    # Reduced: Training batch size (180*8=1440, divisible by 6)
    PPO_MINI_BATCH=180                                      # Reduced: Match train batch size
fi
if [ "$DEBUG" = "True" ]; then
...
# INTUITOR Training - Self-Certainty Based Reinforcement Learning
